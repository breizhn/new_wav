...
%   
% 
%   Y=new_wavread(FILE) reads a WAVE file specified by the string FILE,
%   returning the sampled data in Y. The ".wav" extension is appended
%   if no extension is given.
%
%   [Y,FS,NBITS]=new_wavread(FILE) returns the sample rate (FS) in Hertz
%   and the number of bits per sample (NBITS) used to encode the
%   data in the file.
%
%   [...]=new_wavread(FILE,N) returns only the first N samples from each
%       channel in the file.
%   [...]=WAVREAD(FILE,[N1 N2]) returns only samples N1 through N2 from
%       each channel in the file.
%
%   [Y,...]=new_wavread(...,FMT) specifies the data type format of Y used
...
%
%   SIZ=new_wavread(FILE,'size') returns the size of the audio data contained
%       in the file in place of the actual audio data, returning the
%       2-element vector SIZ=[samples channels].
%
%   ([Y,FS,NBITS,OPTS]=new_wavread(...) returns a structure OPTS of additional
...
if nargout == 4
    error('new_wavread:no_opt', 'struct opt is not implemented')
end
